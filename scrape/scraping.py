...
def scrape_movies(total_movies=5000):
    try:
        conn = mysql.connector.connect(**MYSQL_CONFIG)
        cursor = conn.cursor()
        ...
        start_year = 2021  # Adjust as needed
            ...
            while True:
                try:
                    ...
                    for movie in data["results"]:
                        ...
                        try:
                            # Fetch detailed data (same as before)
                            movie_id = movie["id"]
...
#         while movies_collected < total_movies:
#             try:
#                 # Fetch movie batch
...
#                         # Insert data
#                         if insert_movie_data(cursor, conn, movie_data, watch_providers):
#                             movies_collected += 1
#                             print(
#                                 f"✅ [{movies_collected}/{total_movies}] {movie_data.get('original_title')}"
#                             )

#                     except requests.exceptions.RequestException as e:
#                         print(f"⚠️ API error untuk movie {movie.get('id')}: {e}")
#                     except Exception as e:
#                         print(f"⚠️ Error tidak terduga saat processing movie: {e}")
...
#             except requests.exceptions.RequestException as e:
#                 print(f"⚠️ Gagal Fetch Movie Batch: {e}")
#                 time.sleep(5)
...
if __name__ == "__main__":
    create_tables()
    scrape_movies(total_movies=5000)
